...
Given("el administrador está en la página de etiquetas base", () => {
  cy.wait(1000);
...
When(
  "el administrador crea una nueva etiqueta {string} base",
  (tagName: string) => {
    cy.wait(100);
...
Then(
  "se agrega una etiqueta a la publicacion {string} base",
  (tagName: string) => {
    cy.wait(1000);
